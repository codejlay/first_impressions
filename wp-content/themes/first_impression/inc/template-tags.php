 ...
 */
  ...
  */
 function white_canvas_posted_on() {
  ...
  */
 function white_canvas_posted_by() {
 	$byline = sprintf(
 		esc_html( 'by %s' ),
  ...
  */
 function white_canvas_comment_count() {
  ...
  */
 function white_canvas_entry_footer() {
 	// Hide category and tag text for pages.
 	if ( 'post' === get_post_type() ) {
 		/* translators: used between list items, there is a space after the comma */
 		$categories_list = get_the_category_list( esc_html( ', ' ) );
 		if ( $categories_list && white_canvas_categorized_blog() ) {
  ...
  * @return bool
  */
 function white_canvas_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'white_canvas_categories' ) ) ) {
 		...
 		set_transient( 'white_canvas_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so white_canvas_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so white_canvas_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in white_canvas_categorized_blog.
  */
 function white_canvas_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'white_canvas_categories' );
 }
 add_action( 'edit_category', 'white_canvas_category_transient_flusher' );
 add_action( 'save_post',     'white_canvas_category_transient_flusher' );
  ...
  */
 function white_canvas_comment_list( $comment, $args, $depth ) {
  ...
  */
 function white_canvas_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
